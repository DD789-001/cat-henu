...
export async function isManager(
  reqOrCallback:number|((result:Boolean) => void) = 1,
  callbackReq:number = 1
):Promise<Boolean> {
  const callback = typeof reqOrCallback === 'function' ? reqOrCallback : undefined;
  const req = typeof reqOrCallback === 'function' ? callbackReq : reqOrCallback;
  const result = <Boolean>(await wx.cloud.callFunction({
    name: 'isManager',
    data: {
      req: req
    }
