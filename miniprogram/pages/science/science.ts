// miniprogram/pages/science/science.ts
...
Page({
  ...
  downloadCoverImg() {
    // 先使用配置中的图片；本地资源无需通过云存储下载。
    this.setData({images:config.science_imgs})

    const cloudImages = config.science_imgs
      .map((src, index) => ({src, index}))
      .filter(item => item.src.startsWith('cloud://'));
  ...
  async setImagesList() {
    ...
    for (let index = 0; index < config.science_imgs.length; index++) {
      ...
      await coverImgList.push(coverPath || config.science_imgs[index]);
    }
